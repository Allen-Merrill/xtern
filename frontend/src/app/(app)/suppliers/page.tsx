    ...
    <div style={{ maxWidth: 1100 }}>
      ...
      {!loading && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
          {[
            { label: 'Total Suppliers', value: suppliers.length, accent: 'var(--accent-blue)', css: 'stat-accent-blue' },
            { label: 'Avg Quality Score', value: suppliers.length > 0 ? Math.round(suppliers.reduce((s, x) => s + x.quality_score, 0) / suppliers.length) : 0, accent: 'var(--accent-green)', css: 'stat-accent-green' },
            { label: 'Avg Lead Time', value: suppliers.length > 0 ? `${Math.round(suppliers.reduce((s, x) => s + x.lead_time_days, 0) / suppliers.length)}d` : '\u2014', accent: 'var(--accent-amber)', css: 'stat-accent-amber' },
            { label: 'Total SKUs Covered', value: Object.values(supplierProducts).reduce((s, arr) => s + arr.length, 0), accent: 'var(--accent-purple)', css: 'stat-accent-purple' },
          ].map(s => (
                ...
                >
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{sup.name}</span>
                      <span className={`badge ${regionColor[sup.region] || 'badge-blue'}`}>{sup.region}</span>
                    </div>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div className="mono" style={{ fontSize: 16, fontWeight: 700, color: 'var(--accent-cyan)' }}>{sup.lead_time_days}d</div>
                  </div>
                  <div>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div className="mono" style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)' }}>{products.length}</div>
                  </div>
